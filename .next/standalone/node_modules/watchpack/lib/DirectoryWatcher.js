...
class DirectoryWatcher extends EventEmitter {
	 ...
	 */
	onScanError(err) {
		// [kinexis] suppress harmless Windows scan errors
		if (
			err &&
			IS_WIN &&
			(err.code === "EINVAL" ||
		...
		process.nextTick(() => {
			...
			fs.readdir(this.path, (err, items) => {
				...
				for (const itemPath of itemPaths) {
					// [kinexis] skip protected Windows files during directory scan
					if (IS_WIN) {
						const base = path.basename(itemPath).toLowerCase();
						if (
							base === "pagefile.sys" ||
							base === "swapfile.sys" ||
							base === "hiberfil.sys" ||
							base === "dumpstack.log.tmp"
						) {
							itemFinished();
							continue;
						}
					}
					if (this.ignored(itemPath)) {
						itemFinished();
						continue;
					}
