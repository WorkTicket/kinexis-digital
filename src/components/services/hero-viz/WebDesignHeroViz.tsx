"use client";

import {
  BrowserFrame,
  FloatingBadge,
  HeroVizShell,
  HeroVizStage,
  SkeletonBlock,
  SkeletonLine,
} from "./ServiceHeroVizParts";

export default function WebDesignHeroViz() {
  return (
    <HeroVizShell>
      <HeroVizStage className="items-center">
        <BrowserFrame label="Desktop" delay={0.5} highlight frameClassName="h-[165px] w-[220px]">
          <SkeletonLine width="lg" />
          <SkeletonLine width="md" />
          <div className="mt-auto flex gap-1">
            <SkeletonBlock className="h-5 flex-1 !bg-neon-cyan/15" />
            <div className="h-5 w-8 rounded bg-white/[0.04]" />
          </div>
        </BrowserFrame>

        <FloatingBadge label="PageSpeed" value="94/100" accent="emerald" />
        <FloatingBadge label="Responsive" value="✓" position="bottom-left" delay={1.3} />
      </HeroVizStage>
    </HeroVizShell>
  );
}
