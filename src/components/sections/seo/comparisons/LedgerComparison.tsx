...
export default function LedgerComparison({ columns, rows }: ComparisonProps) {
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-surface bg-gradient-to-br from-white/[0.04] via-bg-secondary/90 to-bg-dark shadow-panel">
      <div className="grid border-b border-surface md:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)_minmax(0,1fr)]">
        <div className="flex items-end border-b border-surface bg-bg-dark px-6 py-5 md:border-b-0 md:border-r">
          <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/35">Compared</p>
        </div>
        <div className="border-b border-surface bg-bg-dark px-6 py-5 md:border-b-0 md:border-r">
      ...
      <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }}>
        {rows.map((row, i) => (
          <motion.div
            ...
            variants={fadeUp}
            className={cn(
              "grid border-b border-surface last:border-0 md:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)_minmax(0,1fr)]",
              i % 2 === 0 ? "bg-transparent" : "bg-surface-base"
            )}
          >
            <div className="border-b border-surface px-6 py-5 md:border-b-0 md:border-r">
              <p className="text-sm font-semibold text-white/80">{row.label}</p>
            </div>
            <div className="flex items-start gap-2.5 border-b border-surface px-6 py-5 md:border-b-0 md:border-r">
